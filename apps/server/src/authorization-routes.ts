...
export function registerAuthorizationRoutes(
  ...
  const currentPolicy = async (actor: AuthorizationActor): Promise<AuthorizationPolicyDto> =>
    toPolicyDto(await authorization.getPolicy(actor));
...
function toPolicyDto(state: AuthorizationPolicyState): AuthorizationPolicyDto {
  return {
    ...
    levels: state.authorityLevels.map((level) => ({
      id: level.id,
