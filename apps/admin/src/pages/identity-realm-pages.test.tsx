...
function renderDetail(
  ...
  mode: DisplayMode = "basic",
  ownerStatus: Record<string, unknown> = { realmId: "rlm_testre", status: "ownerless", policyRevision: 1 },
  existingEntitlements: readonly Record<string, unknown>[] = [],
) {
...
describe("IdentityRealmDetailPage provisioning guidance", () => {
  ...
  it("marks administrator/owner members with a badge and hides the promote button", async () => {
    ...
    const { user } = renderDetail(
      contentRealm({ status: "active", profileCollectionId: "col_profile" }),
      [ownerMember, adminMember, plainMember],
      "basic",
      { status: "healthy", policyRevision: 3, owner: { globalIdentityId: "gid_owner", membershipId: "mem_owner", subjectId: "subject:user:gid_owner", primaryIdentifier: "owner@example.com", identityActive: true, membershipStatus: "active" } },
    );
  ...
  it("previews the bootstrap deadlock: warns to appoint an administrator once an Owner exists", async () => {
    ...
    renderDetail(
      ...
      [],
      "basic",
      { status: "healthy", policyRevision: 3, owner: { globalIdentityId: "gid_o", membershipId: "mem_o", subjectId: "s_o", primaryIdentifier: "owner@example.com", identityActive: true, membershipStatus: "active" } },
    );
  ...
  it("hides the checklist once setup is complete (owner + administrator)", async () => {
    ...
    renderDetail(
      ...
      [adminMembership],
      "basic",
      { status: "healthy", policyRevision: 3, owner: { globalIdentityId: "gid_admin", membershipId: "mem_admin", subjectId: "subj_admin", primaryIdentifier: "admin@example.com", identityActive: true, membershipStatus: "active" } },
    );
...
describe("IdentityRealmDetailPage 표시 모드", () => {
  ...
  it("flags a leftover ceiling on another realm's Auth collection as removable only", async () => {
    ...
    await user.click(await screen.findByRole("tab", { name: "권한" }));
    const strayRow = (await screen.findByText("Portal Accounts")).closest("tr") as HTMLElement;
