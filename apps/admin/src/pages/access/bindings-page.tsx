      ...
      <div className={styles.layout}>
        <div className={styles.stack}>
          <section className={styles.panel}>
            ...
            {policy.data.bindings.length === 0 ? <EmptyState title="배정된 역할이 없습니다" description="사용자 또는 그룹에 첫 역할을 배정해 보세요." /> : <BindingTable policy={policy.data} onEdit={openBinding} advanced={advanced} />}
          </section>
          <section className={styles.panel}>
        ...
        <div className={styles.detailPane} data-open={inspectorOpen}>
          ...
          {subjectCreatorOpen ? (
            ...
            <BindingEditor policy={policy.data} binding={editingBinding} onCancel={closeInspector} onSave={(input) => saveBinding.mutate(input)} onDelete={editingBinding?.id ? () => deleteBinding.mutate(editingBinding.id) : undefined} isPending={saveBinding.isPending || deleteBinding.isPending} advanced={advanced} />
          )}
...
function BindingTable({ policy, onEdit, advanced }: { readonly policy: AuthorizationPolicy; readonly onEdit: (binding: AuthorizationRoleBinding) => void; readonly advanced: boolean }) {
...
function BindingEditor({ policy, binding, onSave, onDelete, onCancel, isPending, advanced }: {
  ...
  return <section className={styles.panel} aria-label="역할 배정 편집기"><SectionHeader title={binding.id ? "역할 배정 수정" : "새 역할 배정"} description="아래 세 단계만 선택하면 역할이 적용됩니다." />{!advanced && hasAdvanced ? <Callout tone="info"><strong>고급 설정 있음</strong> 전파 방식, 기간 또는 조건은 이 화면에서 바뀌지 않으며 저장해도 그대로 유지됩니다.</Callout> : null}<div className={styles.form}>
