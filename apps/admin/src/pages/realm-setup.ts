 ...
 */
export function realmSetupSteps(input: RealmSetupInput): readonly RealmSetupStep[] {
  ...
  const activateStatus: StepStatus = active ? "done" : "current";

  let ownerStatus: StepStatus;
  if (ownerHealthy) ownerStatus = "done";
  else if (!active) ownerStatus = "todo";
  else if (input.ownerCandidateCount === 0) ownerStatus = "blocked";
  else ownerStatus = "current";

  let administratorStatus: StepStatus;
  if (hasAdministrator) administratorStatus = "done";
  else if (!active) administratorStatus = "todo";
  else if (ownerStatus === "current" || ownerStatus === "blocked") administratorStatus = "todo";
  else administratorStatus = "current";

  let accessStatus: StepStatus;
  if (!active || !hasAdministrator) accessStatus = "todo";
  else accessStatus = "current";
...
export function isRealmSetupIncomplete(steps: readonly RealmSetupStep[]): boolean {
