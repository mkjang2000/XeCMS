...
import type { DocumentRecordDto, XeCmsClient } from "@xecms/client";
import { asCollectionId, asFieldId, type SchemaIrV1 } from "@xecms/schema";
import { createAdminApi } from "./client-adapter.js";

const emptySchema: SchemaIrV1 = {
  format: "xecms.schema",
  formatVersion: 1,
  collections: [],
};
...
describe("createAdminApi", () => {
  it("round-trips the exact Collection auth contract through schema drafts", async () => {
    ...
    const savedSchema = saveDraft.mock.calls[0]?.[0].schema as SchemaIrV1;
  ...
  it("materializes M2 hierarchy and relation fields with stable IDs", async () => {
    ...
    const client = {
      schema: {
        getDraft: vi.fn().mockResolvedValue(null),
        getCurrent: vi.fn().mockResolvedValue(null),
        issueIds: vi.fn().mockImplementation(async ({ kind }: { readonly kind: string }) => ({
          ids: kind === "collection"
            ? ["col_pages"]
            : kind === "relation"
              ? ["rel_parent"]
  ...
  it("uses the applied schema for content forms even when a newer draft exists", async () => {
    const appliedSchema: SchemaIrV1 = {
      ...emptySchema,
      collections: [{
  ...
  it("creates a hierarchy root with the latest structure version and tail position", async () => {
    ...
    const created = {
      ...
      createdAt: "2026-07-15T00:00:00.000Z",
      updatedAt: "2026-07-15T00:00:00.000Z",
    } as const satisfies DocumentRecordDto;
  ...
  it("preserves hierarchy permission impact and policy revision CAS across preview and move", async () => {
    const documentRecord: DocumentRecordDto = {
      id: "doc-child",
    ...
    const client = { documents: { previewMove, move } } as unknown as XeCmsClient;
    const api = createAdminApi(client);
    const request = { newParentId: "new-root", position: 1, expectedVersion: 7 } as const;
