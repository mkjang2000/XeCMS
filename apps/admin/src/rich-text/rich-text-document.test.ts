...
function mediaRecord(id: string, contentUrl: string): MediaRecord {
  return {
    id,
    fileName: `${id}.png`,
    mimeType: "image/png",
...
describe("rich text document round trip", () => {
  ...
  it("preserves marks, attrs and nesting through a round trip", () => {
    const content: readonly RichTextNode[] = [
      {
        type: "heading",
        attrs: { level: 2 },
  ...
  it("leaves src empty for a media id that no longer exists", () => {
    const content: readonly RichTextNode[] = [{ type: "image", attrs: { mediaId: "med_gone" } }];
