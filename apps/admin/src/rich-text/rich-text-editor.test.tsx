...
describe("RichTextEditor", () => {
  ...
  it("never reports an empty document while seeding", async () => {
    ...
    render(
      <StrictMode>
        <RichTextEditor field={field} value={storedDocument} onChange={onChange} />
      </StrictMode>,
    );
  ...
  it("keeps the body when the media list refreshes after an upload", async () => {
    ...
    function Host() {
      ...
      return (
        <>
          <button type="button" onClick={() => setItems([media("med_1"), media("med_2")])}>refresh</button>
          <RichTextEditor field={field} value={storedDocument} onChange={vi.fn()} mediaItems={items} />
        </>
      );
    }
  ...
  it("re-seeds after the form resets to a reloaded document", async () => {
    ...
    function Host() {
      ...
      return (
        <>
          <button type="button" onClick={() => setValue(empty)}>clear</button>
          <button type="button" onClick={() => setValue(storedDocument)}>reload</button>
          <RichTextEditor field={field} value={value} onChange={setValue} />
        </>
      );
    }
