import { resolvePreviewQuery } from "@xecms/admin-runtime";
...
export function createPreviewDataClient(pages, fieldNameById = new Map()) {
    ...
    return {
        ...
        queryComposed: async (pageId, dataSourceId, input) => {
            const page = pageById.get(pageId);
            const dataSource = page?.dataSources.find((source) => source.id === dataSourceId);
            if (dataSource === undefined)
                return { items: [], hasNextPage: false };
            const request = resolvePreviewQuery(dataSource, (input.parameters ?? {}), input.cursor);
