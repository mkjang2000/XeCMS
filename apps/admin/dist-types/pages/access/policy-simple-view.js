...
export function ancestorGroupIds(policy, subjectId) {
    const found = new Set();
...
export function subjectGradeState(policy, subjectId) {
    const reasons = [];
    ...
    if (direct.length > 1)
        reasons.push("역할이 여러 개 배정되어 있어요.");
    if (direct.length === 0) {
 ...
 */
export function gradePermissionEdit(role, nextPermissions) {
    return {
        name: role.name,
...
export function droppedDelegations(role, nextPermissions) {
 ...
 */
export function rankBetween(upperRank, lowerRank) {
...
export function memberCountByLevel(policy) {
    ...
    for (const binding of policy.bindings) {
        ...
        if (levelId === undefined)
            continue;
        const set = subjectsByLevel.get(levelId) ?? new Set();
 ...
 */
export function gradeOptions(policy) {
    return sortedLevels(policy).flatMap((level) => {
        const state = levelSimpleState(policy, level);
...
//# sourceMappingURL=policy-simple-view.js.map