...
import { ancestorGroupIds, droppedDelegations, gradeOptions, gradePermissionEdit, levelSimpleState, memberCountByLevel, rankBetween, rootResource, subjectGradeState, } from "./policy-simple-view.js";
...
describe("levelSimpleState", () => {
    it("classifies the seed ladder: protected / aggregate / editable / empty", () => {
        const policy = seedPolicy();
        const level = (id) => policy.levels.find((item) => item.id === id);
...
describe("subjectGradeState", () => {
    ...
    it("flags scoped, constrained, or timed bindings as complex", () => {
        ...
        const scoped = seedPolicy({
            bindings: policy.bindings.map((item) => item.id === "binding-editor" ? { ...item, resourceId: "content", propagation: "self" } : item),
        });
        ...
        const timed = seedPolicy({
            bindings: policy.bindings.map((item) => item.id === "binding-editor" ? { ...item, validUntil: "2026-12-31T00:00:00.000Z" } : item),
        });
        ...
        const constrained = seedPolicy({
            bindings: policy.bindings.map((item) => item.id === "binding-editor" ? { ...item, constraints: { statuses: ["draft"] } } : item),
        });
        ...
        const multi = seedPolicy({
            bindings: [...policy.bindings, { id: "binding-extra", realmId: "system", subjectId: "subject-editor", roleId: "role-viewer", resourceId: "root", propagation: "self-and-children", protected: false }],
        });
...
//# sourceMappingURL=policy-simple-view.test.js.map