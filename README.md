...
| 영역 | 지원 상태 |
| --- | --- |
| 저장소 개요와 빠른 시작 | 한국어 기본, [영문 README](./README.en.md) 제공 |
| 상세 `docs/` 문서 | 현재 한국어 제공 |
| CMS Studio와 내부 관리 UI | 현재 한국어만 지원 (UI 다국어화 미지원) |
| CLI, API, 설정과 Schema | 영문 기술 식별자 사용 |

위 표는 XeCMS가 기본 제공하는 문서와 관리 화면의 언어 범위를 나타낸다. 애플리케이션에서
관리하는 콘텐츠의 언어와 현지화 정책은 각 프로젝트에서 구성한다.
