...
const serverUrl = process.env.XECMS_M4A_SERVER_URL ?? "http://127.0.0.1:3120";
const databaseUrl = process.env.XECMS_M4A_DATABASE_URL ?? "postgresql://xecms:xecms@127.0.0.1:55432/xecms_e2e";
const mediaStorageRoot = await mkdtemp(join(tmpdir(), "xecms-m4a-media-"));
const environment = {
  ...process.env,
  ...
  XECMS_DB_SCHEMA: process.env.XECMS_M4A_DB_SCHEMA ?? `xecms_m4a_e2e_${process.pid}_${Date.now()}`,
  XECMS_RUN_POSTGRES_TESTS: "true",
  XECMS_E2E_AUTO_BOOTSTRAP: "true",
  XECMS_E2E_OWNER_PASSWORD: "Admin-test-only-2026!",
  XECMS_SESSION_SECRET: "m4a-e2e-only-session-secret-change-me",
...
try {
  ...
  await run(pnpm, ["exec", "vitest", "run", "packages/database/src/postgres-schema-auth-materialization.test.ts", "apps/server/src/m4a-identity-realms.integration.test.ts"]);
  await run(pnpm, ["exec", "playwright", "test", "tests/e2e/m4a-identity-realms.spec.ts"]);
} finally {
