...
function plugin(id="alpha",dependencies:Record<string,string>={}):XeCmsPluginModule{return{manifest:{manifestVersion:1,id,packageName:`@test/${id}`,version:"1.0.0",displayName:id,compatibility:{core:">=0.5.0 <0.6.0",admin:">=0.5.0 <0.6.0",sdk:"1.x"},dependencies}}}
describe("Plugin SDK manifest",()=>{
  ...
  it("produces a key-order-independent manifest digest",()=>{const first=plugin().manifest;const second={...first,compatibility:{sdk:"1.x",admin:">=0.5.0 <0.6.0",core:">=0.5.0 <0.6.0"}};expect(pluginManifestDigest(first)).toBe(pluginManifestDigest(second))});
});
