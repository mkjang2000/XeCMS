...
export async function scaffold(directory:string,starter:StarterName):Promise<string>{const root=resolve(directory);await mkdir(root,{recursive:true});if((await readdir(root)).length>0)throw cliError("DIRECTORY_NOT_EMPTY",`Refusing to initialize non-empty directory '${root}'.`);const projectName=safeName(basename(root)),artifacts=starterArtifacts(starter);const files:Record<string,string>={
  "package.json":`${JSON.stringify({name:projectName,version:"0.1.0",private:true,type:"module",packageManager:"pnpm@10.30.1",engines:{node:">=22.22.0"},scripts:{dev:"xecms dev",start:"xecms start",build:"xecms schema validate && xecms generate types",check:"xecms schema validate",migrate:"xecms migrate",doctor:"xecms doctor",backup:"xecms backup create ./backups/manual"},dependencies:{"@xecms/cli":"^0.5.0","@xecms/server":"^0.5.0","@xecms/admin-app":"^0.5.0"}},null,2)}\n`,
...
function compose(){return`name: xecms-project\nservices:\n  postgres:\n    image: postgres:18-alpine\n    environment:\n      POSTGRES_DB: xecms\n      POSTGRES_USER: xecms\n      POSTGRES_PASSWORD: xecms\n    ports:\n      - "54320:5432"\n    volumes:\n      - postgres-data:/var/lib/postgresql\n    healthcheck:\n      # Probe TCP, not the unix socket: while Postgres initialises it serves the\n      # socket before opening TCP, so a socket-only check reports healthy while\n      # clients on the published port still get ECONNREFUSED.\n      test: ["CMD-SHELL", "pg_isready -h 127.0.0.1 -p 5432 -U xecms -d xecms"]\n      interval: 2s\n      timeout: 5s\n      retries: 20\n      start_period: 5s\nvolumes:\n  postgres-data:\n`}
function readme(name:string,starter:StarterName){return`# ${name}\n\nXeCMS **${starter}** starter입니다.\n\n## 개발\n\n\`\`\`bash\npnpm install\ndocker compose up -d --wait\npnpm migrate\npnpm dev\n\`\`\`\n\nAdmin: http://127.0.0.1:3100/admin/setup\n\n최초 실행 시 Setup 화면에서 Owner 계정을 생성합니다.\n\n## 프로덕션 실행\n\n\`.env\`의 DB URL, 공개 Origin과 session secret을 운영 값으로 교체한 뒤 실행합니다.\n\n\`\`\`bash\npnpm build\npnpm migrate\npnpm start\n\`\`\`\n\nSchema를 수정한 후 \`pnpm build\`, 운영 점검은 \`pnpm doctor\`를 실행합니다.\n`}
