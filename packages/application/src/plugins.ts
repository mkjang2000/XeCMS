...
export class PluginService{
  ...
  public async assertStartupState(workspaceId: string): Promise<readonly PluginRecord[]> {
    const records = await this.store.list(workspaceId);
    for (const record of records) {
      if (record.desiredState !== "enabled") continue;
      const recovery = `Run 'xecms plugin inspect' and 'xecms plugin disable ${record.pluginId} --offline', then restart.`;
      if (!this.catalog.has(record.pluginId)) {
        throw new ApplicationError("PLUGIN_ENABLED_PACKAGE_MISSING", 503, `Enabled Plugin '${record.pluginId}' is missing from the runtime catalog. ${recovery}`);
      }
      const module = this.catalog.get(record.pluginId);
      if (pluginManifestDigest(module.manifest) !== record.manifestDigest) {
        throw new ApplicationError("PLUGIN_MANIFEST_DRIFT", 503, `Enabled Plugin '${record.pluginId}' manifest changed without an install plan. Run 'xecms plugin sync' and 'xecms plugin sync --apply' for matching migration history. For database-only recovery: ${recovery}`);
      }
      try {
        await this.store.assertMigrationState(workspaceId, module);
      } catch (error) {
        if (error instanceof ApplicationError && error.code === "PLUGIN_MIGRATION_CHECKSUM_DRIFT") {
          throw new ApplicationError(error.code, error.status, `${error.message} ${recovery} Restore matching migration history before enabling.`, error.options);
        }
        throw error;
      }
    }
    return records.filter(record => record.desiredState === "enabled");
  }
